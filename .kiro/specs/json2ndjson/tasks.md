...
- **1 レコードのバイト数に対しヒープのピークは厳密に 5.0 倍**(8 / 16 / 32 / 64 MiB のいずれでも 5.0x。`TestPerfRecordHeapMultiplier` が報告)。したがって**要件 9.2 / NFR-02 の上限 256 MB を破るのは 1 レコードが約 51 MiB を超えたとき**で、64 MiB のレコードでは実測 320 MiB に達する。
